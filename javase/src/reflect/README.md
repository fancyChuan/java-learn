...
静态代理：
- 组成：目标接口、目标接口实现类、代理类（需要实现目标接口）

动态代理：
- 组成：目标接口、目标接口实现类、实现了InvocationHandler的类

> 动态代理跟静态代理相比：代理类不需要实现接口，从而不需要跟接口一一对应。在需要对多个接口进行代理(比如在方法调用前统一检查权限-也就是进行功能增强)时，动态代理只需要一份代码，就可以批量对多个接口进行代理。也可以说就是面向横切面编程AOP了！