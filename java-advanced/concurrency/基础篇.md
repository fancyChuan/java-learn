...
- 死锁产生的原因：
    ...
    - 必须具备的四个条件
        - 互斥条件。线程对已经获取到的资源进行排他使用。也就是同一资源同时只能由一个线程占用
        - 请求并持有条件。指一个线程已经持有至少一个资源，但有提出新的资源请求，而新的资源被其他线程持有，导致当前线程阻塞的同时并不释放自己已经持有的资源
        - 不可剥夺条件。指线程获取到的资源在自己使用完之前不能被其他线程抢占，只能由自己释放
        - 环路等待条件。指发生死锁时必然存在一个线程-资源的环形链。比如t0等待t1、t1等待t2、t2等待t0
- 避免死锁的常见方法：
    - 避免一个线程同时获取多个锁
